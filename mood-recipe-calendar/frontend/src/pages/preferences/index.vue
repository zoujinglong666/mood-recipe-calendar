...
import { clearFoodPreference, fetchFoodPreference, saveFoodPreference, type SpiceLevel } from '../../api/preferences'
...
function finish() {
  ...
  if (onboarding.value && mood) router.replace({ name: 'recipe', query: { mood } })
  else navBack()
}
...
<template>
  <view class="memory-page">
    <wd-navbar title="锅仔记忆" left-arrow safe-area-inset-top @click-left="navBack" custom-style="background-color: transparent !important;" />

    <view v-if="loading" class="memory-loading">锅仔正在翻开记忆本…</view>
    <template v-else>
      <view class="memory-hero">
        ...
        <view class="memory-hero__copy">
          <text class="memory-hero__eyebrow">只记你愿意告诉我的</text>
          <text class="memory-hero__title">以后每一餐，都更懂你一点</text>
          <text class="memory-hero__body">这些记忆只用于推荐，你可以随时修改或清除。</text>
        </view>
      </view>

      <view class="memory-section">
        <text class="memory-section__title">你平时喜欢吃什么？</text>
        <text class="memory-section__hint">可以多选，之后还能慢慢调整</text>
        ...
        <text class="memory-section__hint memory-section__hint--input">也可以直接告诉锅仔你爱吃的菜</text>
        <input v-model="favoriteDishes" class="memory-input" :maxlength="500" placeholder="例如：番茄炒蛋、红烧肉" />
      </view>

      <view class="memory-section">
        <text class="memory-section__title">两件很重要的小事</text>
        <view class="binary-row">
          <text class="binary-row__label">吃葱吗？</text>
          <view class="binary-row__actions">
            <view class="binary-button" :class="{ 'binary-button--selected': eatScallion === true }" @click="eatScallion = true">可以</view>
            <view class="binary-button" :class="{ 'binary-button--selected': eatScallion === false }" @click="eatScallion = false">不要</view>
          </view>
        </view>
        <view class="binary-row">
          <text class="binary-row__label">吃香菜吗？</text>
          <view class="binary-row__actions">
            <view class="binary-button" :class="{ 'binary-button--selected': eatCilantro === true }" @click="eatCilantro = true">可以</view>
            <view class="binary-button" :class="{ 'binary-button--selected': eatCilantro === false }" @click="eatCilantro = false">不要</view>
          </view>
        </view>
      </view>

      <view class="memory-section">
        <text class="memory-section__title">能吃多辣？</text>
      ...
      <view class="memory-section">
        <text class="memory-section__title">还有什么不吃？</text>
        <text class="memory-section__hint">用逗号隔开，例如：芹菜、花生、动物内脏</text>
      ...
      <view class="memory-section memory-section--warning">
        <text class="memory-section__title">食物过敏</text>
        <text class="memory-section__hint">例如：花生、虾、乳制品。推荐仅作辅助，食用前仍需核对全部食材。</text>
        <input v-model="allergens" class="memory-input" :maxlength="500" placeholder="没有可以留空" />
      </view>

      <view class="memory-actions">
        <button class="memory-save" :disabled="saving" @click="save">{{ saving ? '正在记住…' : '让锅仔记住' }}</button>
        <button v-if="onboarding" class="memory-skip" :disabled="saving" @click="skip">暂时跳过</button>
        <button v-else class="memory-clear" :disabled="saving" @click="clearMemory">清除全部口味记忆</button>
      </view>
    </template>
  </view>
</template>
...
.choice-chip, .binary-button { min-height: 80rpx; display: flex; align-items: center; justify-content: center; box-sizing: border-box; padding: 0 12rpx; border: 2rpx solid var(--mrc-border); border-radius: 40rpx; background: var(--mrc-bg); color: var(--mrc-text-deep); font-size: 24rpx; font-weight: 700; transition: transform .15s ease, background-color .2s ease; }
...
.binary-button { min-width: 110rpx; min-height: 68rpx; }
